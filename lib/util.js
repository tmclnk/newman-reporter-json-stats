var fs = require("fs"),
  ...
  UID_REGEX =
    /^[0-9A-Z]+-[0-9A-F]{8}-[0-9A-F]{4}-[0-9A-F]{4}-[0-9A-F]{4}-[0-9A-F]{12}$/i;
...
util = {
  ...
  fetchJson: function (type, location, options, callback) {
    ...
    if (
      ...
      POSTMAN_API_PATH_MAP[type] &&
      postmanApiKey &&
      UID_REGEX.test(location)
    ) {
    ...
    return /^https?:\/\/.*/.test(location)
          ...
          (err, response, body) => {
            if (err) {
              return callback(
                _.set(
                  err,
                  "help",
                  `unable to fetch data from url "${location}"`
                )
              );
            }
            ...
            try {
              ...
              return callback(
                _.set(
                  e,
                  "help",
                  `the url "${location}" did not provide valid JSON data`
                )
              );
            }
            ...
            if (response.statusCode !== 200) {
              ...
              urlObj.hostname === POSTMAN_API_HOST &&
                (resource =
                  _(urlObj.pathname).split("/").get(1).slice(0, -1) ||
                  resource);
              ...
              return callback(
                _.assign(error, {
                  name: _.get(
                    body,
                    "error.name",
                    _.capitalize(resource) + "FetchError"
                  ),
      ...
      : fs.readFile(location, function (err, value) {
          if (err) {
            return callback(
              _.set(err, "help", `unable to read data from file "${location}"`)
            );
          }

          try {
            value = liquidJSON.parse(
              value.toString(util.detectEncoding(value)).trim()
            );
          } catch (e) {
            return callback(
              _.set(
                e,
                "help",
                `the file at "${location}" does not contain valid JSON data`
              )
            );
          }
