...
module.exports = function (emitter, reporterOptions, collectionRunOptions) {
  ...
  emitter.on("done", function (err, o) {
    // place statistics somewhere they can be accessed as a library
    o.statistics = statistics;
    // don't dump output if silent
    if (!collectionRunOptions.silent) {
      console.log(JSON.stringify(statistics, 0, 2));
    }
  });
};
